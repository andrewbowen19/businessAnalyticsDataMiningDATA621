...
Based on the definitions of `TEAM_BATTING_H`, `TEAM_BATTING_2B`, `TEAM_BATTING_3B`, and `TEAM_BATTING_HR`, there is probably some multicollinearity goung on with these variables.

```{r}
lm_all_reduced_hits <- update(lm_all_reduced, .~. - TEAM_BATTING_2B - TEAM_BATTING_3B - TEAM_BATTING_HR)
summary(lm_all_reduced_hits)

lm_all_reduced_spec <- update(lm_all_reduced, .~. - TEAM_BATTING_H)
summary(lm_all_reduced_spec)
```
...
The Q-Q plot shows that the residuals of this model are fairly normally distributed. The residuals vs. fitted plot shows a cluster of residuals and a seeming outlying point. The round cluster would suggest that homoscedasticity is satisfied in this model. 
