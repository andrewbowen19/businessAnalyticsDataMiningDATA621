...
>>>>>>> 7a4bf3696a7dfff5965afb47fac1bbb30eb229c7
...
=======
Overall, we saw somewhat comparable RMSE values between our models, indicating similar performance from the initial evaluation dataset using this metric. In terms of getting approval for the decisions made from our manager we would likely select the reduced model as it evaluated the variations that were possible and from a statistical perspective used AIC, Akaike information criterion, as a means for it's decision making. That might not be convincing enough for some managers but we would explain that the combinations of some of the predictors was able to minimize our loss function and the statistic, AIC, is meant to balance the benefits of linear combinations and model complexity. If our manager wanted us to prioritize our judgement/intuition over the statistics to more closely justify the predictors in the model then we would use the offensive model in any future forecasting needed.
