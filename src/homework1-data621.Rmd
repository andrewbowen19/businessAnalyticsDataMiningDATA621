...
predict_all_transformed <- predict(lm_all_transformed, test)
```
...
rmse_all_transformed <- rmse(predict_all_transformed, test$TARGET_WINS)
...
ggplot(as.data.frame(predict_reduced_eval), aes(x = predict_reduced_eval)) + geom_histogram(bins=15) + labs(x="Wins", title="Predicted Wins (Reduced Model): Evaluation Data.")
```

Roughly speaking, these predicted wn totals look roughly normal, and centered around 80 wins, which is expected
