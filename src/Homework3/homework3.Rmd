...
```
The exponentiated coefficients from both Model 3a and 3b show that there is a substantial odds change incorporating binned tax ranges with both AIC and BIC criterion metrics. From the initial exploratory analysis with box-plots the higher crime rate locations had a very large interquartile range. These tax related predictors do not follow expectations given that we expected higher property tax rates to correspond to less crime. That inverse relationship was also present in the linear comparison in EDA so the logistic regression model did not alter the relationship in the dataset, but we wonder how well that would extrapolate to unseen data. Perhaps additional insight into the process for calculating the rate would help explain this relationship further.
...
The `caret` package makes it very easy to calculate the metrics (shown above) needed to evaluate binary logistic regression models and although all of the standard ones from this package are provided we are going to focus on a smaller subset of them for our analysis. We would like select the model that minimizes the risk of incorrectly classifying a neighborhood with elevated crime rates. The reason we are primarily concerned with minimizing false positives is because some of the best ways to combat crime would be to allocate money, resources and programming to that area. There is only so much budget available for city planners and policy makers and therefore it would be best to correctly classify the neighborhoods that will need this assistance to help combat/lower crime.

Based on the criteria stated above it makes most sense to maximize positive prediction values, accuracy, as well as balance precision/recall given those two metrics emphasize the accuracy and completeness of positive predictions. Ultimately the F1 score is used to find a balance between those two metrics and therefore may be slightly easier way to compare the models against one another. Model 2 has the highest F1 Score, accuracy, and AUC which we think makes it the best choice to meet our needs and those of policy makers to identify neighborhoods with above average crime rates.
...
Given that Model 2 has transformed predictors we plotted the evaluation set distributions to confirm they are similar enough to apply the same transformations. It does seem as though the predictors used in Model 2 have any substantial differences between testing and training sets.
