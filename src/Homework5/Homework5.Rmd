...
print(glue("Only {pull(na_freq |> filter(num_na>1)|>summarise(total=n()))} rows have more than 1 column with NA, which is only {format(round(pull(na_freq |> filter(num_na>1)|>summarise(total=n())/nrow(na_freq))*100,2),nsmall=2)}% of the observations that have NAs"))
...
As indicated in the exploratory section of this analysis the negative variables are fairly commonplace and present a unique challenge given the limitations around exclusion. Therefore, we will begin the modeling utilizing the remaining predictors available that did not have illogical negative values. 
...
mod2_zip <- pscl::zeroinfl(formula=TARGET ~ STARS + LabelAppeal + AcidIndex |STARS + LabelAppeal,data=train_df_imputed_input,dist='poisson')
...
mod4_zinb <- pscl::zeroinfl(formula=TARGET ~ STARS + LabelAppeal + AcidIndex| STARS + LabelAppeal,data=train_df_imputed_input,dist='negbin')
...
```{r mod6,echo=F}
mod6_mlr <- lm(TARGET.TF ~ STARS + LabelAppeal + pH + Density + AcidIndex - TARGET,data=train_df_imputed_input)
...
holdout_zi <- as.data.frame(cbind(test_df_imputed_input,zip_pred,zinb_pred,zip_zero,zinb_zero)) |> dplyr::select(c(TARGET,zip_pred,zinb_pred,zip_zero,zinb_zero)) |> mutate(zip_pred_rnd= ifelse(zip_zero>=0.5,0,round(zip_pred,0)),zinb_pred_rnd=ifelse(zinb_zero>=0.5,0,round(zinb_pred,0)),zip_match=ifelse(zip_zero>0.5,ifelse(TARGET==0,1,0),ifelse(TARGET==zip_pred_rnd,1,0)),zinb_match=ifelse(zinb_zero>0.5,ifelse(TARGET==0,1,0),ifelse(TARGET==zinb_pred_rnd,1,0)),zip_res=TARGET-zip_pred_rnd,zinb_res=TARGET-zinb_pred_rnd,zip_fact=as.factor(zip_match),zinb_fact=as.factor(zinb_match),is_zip_zero=ifelse(zip_zero>0.5,'0',as.character(zip_pred_rnd)),is_zinb_zero=ifelse(zinb_zero>0.5,'0',as.character(zinb_pred_rnd)),is_zero=ifelse(TARGET==0,'Y','N'))

```

The process for determining the correct prediction is to first determine if the logistic model classified zero values. After rounding the predicted values to the closest discrete value for both variations of the model it appears that the the ZIP and ZINB models are returning identical predictions for the holdout set. While the expected values are slightly different to the decimal place that has not impacted the end discrete prediction.

Each model's accuracy is not that strong at 1627/3813 ~ 42.67%


```{r accur_metric,echo=F}
knitr::kable(cbind(holdout_zi |> group_by(zip_match) |> summarize(cnt=n()),holdout_zi |> group_by(zinb_match) |> summarize(cnt=n())), format = "simple")
```
...
Based on all of the above criteria, it would appear that the Zero Inflated Poisson just edges at the Zero Inflated Negative Binomial given it's slightly lower AIC metric. All the other criteria stacks up fairly evently between these two models.

### Evaluation

Let's check for missing values in the evaluation set:

```{r missing_eval_data, echo=F}
...
The final predicted distribution appears to expect less zero values than what was in the training dataset; however, it is in line what prior data and appears to follow the general shape of a Poisson distribution.
