...
We'll create a 20% holdout set of data to perfomr model evaluation on later
```{r train-test-split}
set.seed(42)
...
First, we can construct a linear model
...
As we see from summary stats below $Rating \rightarrow English/Math$ models perform decently well at predicting ACT English and Math scores, respectively.
...
We can use two variables as a proxy for the school's survey rating
...
```{r}
proxy_formula <- val_mean_score_act_engl_all ~ eni_hs_pct_912 + temp_housing_pct
lm_proxy <- lm(proxy_formula, data=train)
summary(lm_proxy)
```
