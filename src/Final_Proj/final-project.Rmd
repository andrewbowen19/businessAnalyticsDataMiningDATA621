...
From our diagnostic plots of the WLS model above, we see no general pattern in the *Residuals vs Fitted* plot. In addition, our QQ plot shows general normality behavior, with some behavior off the trendline near the tails of the distribution. Similar to above, we can run a Shapiro test on the residuals of the WLS model to assess normality of residuals, with the same alternative and null hypotheses as above:
...
In the lasso model, `temp_housing_pct` and `economic_need` are more important than any race or gender percentage variables, unlike in the SVM:RB model.
...
rmse_summ <- c(rmse(rating_model, test),
               modelr::rmse(proxy_model, test),
               modelr::rmse(wls_model, test))
rmse_summ <- as.data.frame(cbind(model_names, rmse_summ))
cols <- c("Model", "RMSE")
colnames(rmse_summ) <- cols
apa_table(rmse_summ, placement = "H")
