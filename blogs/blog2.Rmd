...
We can use the built-in `iris` dataset, which is a good sample dataset for classification tasks. In this case, we'll want to predict the multiclass `Species` variable
```{r}
data("iris")
head(iris)
```
